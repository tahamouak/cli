...
func NewCmdPrs(f *cmdutil.Factory, runF func(*shared.IssuesOptions) error) *cobra.Command {
    ...
    `),
		RunE: func(c *cobra.Command, args []string) error {
			...
			if c.Flags().Changed("merged") && merged {
	...
	cmdutil.StringSliceEnumFlag(cmd, &opts.Query.Qualifiers.Is, "visibility", "", nil, []string{"public", "private"}, "Filter based on visibility")
	...
	cmd.Flags().StringVar(&opts.Query.Qualifiers.Milestone, "milestone", "", "Filter by milestone")
	cmdutil.StringEnumFlag(cmd, &opts.Query.Qualifiers.No, "no", "", "", []string{"label", "milestone", "assignee", "project"}, "Filter based on missing metadata")
	...
	cmd.Flags().StringVar(&opts.Query.Qualifiers.Repo, "repo", "", "Filter on repository")
	...
	cmd.Flags().StringVar(&opts.Query.Qualifiers.Base, "base-branch", "", "Filter on base branch name")
	cmdutil.NilBoolFlag(cmd, &opts.Query.Qualifiers.Draft, "draft", "", "Filter based on draft state")
	cmd.Flags().StringVar(&opts.Query.Qualifiers.Head, "head-branch", "", "Filter on head branch name")
